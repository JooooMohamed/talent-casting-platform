...
export class TalentService {
  ...
  async createOrUpdateProfile(userId: string, dto: CreateTalentProfileDto): Promise<TalentProfileDocument> {
  ...
  async getPublicProfile(slug: string, viewerId?: string): Promise<TalentProfileDocument> {
    ...
    this.talentModel.findByIdAndUpdate(profile._id, { $inc: { profileViews: 1 } }).exec();
  ...
  async browseMarketplace(filters: TalentFilters) {
    ...
    if (filters.language) query['languages.language'] = { $regex: filters.language, $options: 'i' };
    if (filters.ageMin || filters.ageMax) {
      const now = new Date();
      query.dateOfBirth = {};
      if (filters.ageMin) {
        query.dateOfBirth.$lte = new Date(now.getFullYear() - filters.ageMin, now.getMonth(), now.getDate());
      }
      if (filters.ageMax) {
        query.dateOfBirth.$gte = new Date(now.getFullYear() - filters.ageMax, now.getMonth(), now.getDate());
      }
    }
    ...
    const [items, total] = await Promise.all([
      this.talentModel
        .find(query)
        .select('fullName slug profilePhoto gender city country categories experience availability isVerified isFeatured')
  ...
  async updateProfilePhoto(userId: string, asset: { url: string; publicId: string }): Promise<void> {
    await this.talentModel.findOneAndUpdate({ userId }, { profilePhoto: asset });
  }
  ...
  async updateVideo(
    ...
    await this.talentModel.findOneAndUpdate({ userId }, {
  ...
  async addPortfolioVideo(
    ...
    if (profile.portfolioVideos.length >= 5) {
      throw new BadRequestException('Maximum 5 portfolio videos allowed on Pro plan');
    }
    profile.portfolioVideos.push({ ...asset, status: 'pending' } as any);
    return profile.save();
  }

  async removePortfolioVideo(userId: string, videoPublicId: string): Promise<void> {
  ...
  async getPendingProfiles(page: number, limit: number) {
    const skip = (page - 1) * limit;
    const [items, total] = await Promise.all([
      this.talentModel.find({ approvalStatus: ApprovalStatus.PENDING }).skip(skip).limit(limit).sort({ createdAt: 1 }),
      this.talentModel.countDocuments({ approvalStatus: ApprovalStatus.PENDING }),
    ]);
    return { items, total, page, limit, totalPages: Math.ceil(total / limit) };
  }
  ...
  async rejectProfile(profileId: string, reason: string): Promise<TalentProfileDocument> {
    const profile = await this.talentModel.findByIdAndUpdate(
      profileId,
  ...
  async toggleFeatured(profileId: string, isFeatured: boolean): Promise<TalentProfileDocument> {
    const profile = await this.talentModel.findByIdAndUpdate(
      profileId,
      { isFeatured },
      { new: true },
  ...
  async toggleVerified(profileId: string, isVerified: boolean): Promise<TalentProfileDocument> {
    const profile = await this.talentModel.findByIdAndUpdate(
      profileId,
      { isVerified },
      { new: true },
  ...
  async getStats() {
    const [total, approved, pending, featured] = await Promise.all([
      ...
      this.talentModel.countDocuments({ approvalStatus: ApprovalStatus.APPROVED }),
      this.talentModel.countDocuments({ approvalStatus: ApprovalStatus.PENDING }),
