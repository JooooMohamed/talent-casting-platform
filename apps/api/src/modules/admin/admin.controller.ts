...
export class AdminController {
  ...
  async toggleVerified(
    @Param("id") id: string,
    @Body() dto: ToggleVerifiedDto,
  ) {
    const profile = await this.talentService.toggleVerified(id, dto.isVerified);
    return { success: true, data: profile };
  }

  @Get("talents/pending-media")
  @ApiOperation({ summary: "List talents with pending media awaiting review" })
  async pendingMedia(@Query("page") page = 1, @Query("limit") limit = 20) {
    const result = await this.talentService.getPendingMedia(+page, +limit);
    return { success: true, data: result };
  }

  @Put("talents/:id/media/approve")
  @ApiOperation({ summary: "Approve a media item for a talent" })
  async approveMedia(
    @Param("id") id: string,
    @Body() body: { mediaType: string; mediaIndex?: number },
  ) {
    const profile = await this.talentService.approveMedia(
      id,
      body.mediaType,
      body.mediaIndex,
    );
  ...
  @Put("talents/:id/media/reject")
  @ApiOperation({ summary: "Reject a media item for a talent" })
  async rejectMedia(
    @Param("id") id: string,
    @Body() body: { mediaType: string; reason: string; mediaIndex?: number },
  ) {
    const profile = await this.talentService.rejectMedia(
      id,
      body.mediaType,
      body.reason,
      body.mediaIndex,
