...
const STATUS_META: Record<string, { bg: string; text: string; icon: string; label: string }> = {
  approved: { bg: colors.greenLight,  text: colors.green,         icon: '✅', label: 'Profile Approved' },
  pending:  { bg: colors.yellowLight, text: colors.yellow,        icon: '⏳', label: 'Pending Review' },
  rejected: { bg: colors.redLight,    text: colors.red,           icon: '❌', label: 'Profile Rejected' },
};

function QuickAction({ icon, title, sub, onPress, accent }: any) {
  return (
    <TouchableOpacity style={[qa.wrap, accent && qa.accent]} onPress={onPress} activeOpacity={0.82}>
  ...
  return (
    <ScrollView style={s.root} showsVerticalScrollIndicator={false}>
      ...
      <View style={s.hero}>
        ...
        {/* Name + email */}
        <Text style={s.heroName}>{profile?.fullName ?? 'Set up your profile'}</Text>
        ...
        {(profile?.city || profile?.experience) && (
          <View style={s.heroBadgeRow}>
            {profile?.city && (
              <View style={s.heroBadge}><Text style={s.heroBadgeText}>📍 {[profile.city, profile.country].filter(Boolean).join(', ')}</Text></View>
            )}
            {profile?.experience && (
              <View style={s.heroBadge}><Text style={s.heroBadgeText}>🏆 {profile.experience}</Text></View>
            )}
          </View>
        )}
        ...
        {statusMeta && (
          <View style={[s.statusBadge, { backgroundColor: statusMeta.bg }]}>
            ...
            {profile?.rejectionReason && (
              <Text style={[s.statusReason, { color: statusMeta.text }]}> — {profile.rejectionReason}</Text>
            )}
          </View>
        )}
      </View>
      ...
      <View style={s.qaGrid}>
        ...
        <QuickAction
          ...
          onPress={() => profile?.slug && navigation.navigate('TalentProfile', { slug: profile.slug })}
        />
      </View>
      ...
      {profile && (
        <View style={s.completeCard}>
          ...
          <View style={s.progressBg}>
            <View style={[s.progressFill, { width: `${getCompleteness(profile)}%` }]} />
          </View>
          <Text style={s.completeHint}>{getCompleteness(profile)}% complete — {getCompletenessTip(profile)}</Text>
        </View>
      )}
...
function getCompleteness(p: any): number {
  const checks = [p.fullName, p.bio, p.city, p.experience, p.profilePhoto, p.introVideo, (p.skills?.length > 0), (p.languages?.length > 0)];
...
const s = StyleSheet.create({
  ...
  heroCircle1: {
    ...
    backgroundColor: 'rgba(139,92,246,0.18)', top: -50, right: -50,
  },
  heroCircle2: {
    position: 'absolute', width: 150, height: 150, borderRadius: 75,
    backgroundColor: 'rgba(109,40,217,0.2)', bottom: -40, left: -30,
  },
  ...
  heroName: { fontSize: 22, fontWeight: '800', color: '#fff', textAlign: 'center', letterSpacing: -0.3 },
  ...
  heroBadge: { backgroundColor: 'rgba(255,255,255,0.13)', borderRadius: radius.full, paddingHorizontal: 12, paddingVertical: 4 },
  heroBadgeText: { fontSize: 12, color: '#fff', fontWeight: '500', textTransform: 'capitalize' },
  ...
  sectionTitle: { fontSize: 16, fontWeight: '700', color: colors.gray[900], marginHorizontal: spacing.md, marginTop: spacing.lg, marginBottom: spacing.sm },

  /* Quick action grid */
  qaGrid: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: spacing.md, gap: spacing.sm },

  /* Completeness */
  completeCard: {
    backgroundColor: '#fff', margin: spacing.md, borderRadius: radius.lg,
    padding: spacing.md,
    shadowColor: colors.brand[800], shadowOpacity: 0.05, shadowRadius: 8, elevation: 2,
  },
  completeTitle: { fontSize: 14, fontWeight: '700', color: colors.gray[800], marginBottom: 10 },
  progressBg: { height: 8, backgroundColor: colors.gray[100], borderRadius: 4, overflow: 'hidden', marginBottom: 8 },
  progressFill: { height: '100%', backgroundColor: colors.brand[600], borderRadius: 4 },
  completeHint: { fontSize: 12, color: colors.gray[500] },

  logoutBtn: { alignItems: 'center', paddingVertical: spacing.lg, marginBottom: spacing.xl },
...
const qa = StyleSheet.create({
  wrap: {
    ...
    padding: spacing.md,
    shadowColor: colors.brand[800], shadowOpacity: 0.06, shadowRadius: 8, elevation: 2,
  },
  accent: {
