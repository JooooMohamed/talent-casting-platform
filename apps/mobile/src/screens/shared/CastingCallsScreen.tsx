...
function getRoleIcon(roleType = '') {
  ...
  for (const k of Object.keys(ROLE_ICONS))
    if (l.includes(k)) return ROLE_ICONS[k];
...
function DeadlineTag({ deadline }: { deadline?: string }) {
  ...
  if (days < 0)
    return (
      <View style={[tag.wrap, tag.expired]}>
        <Text style={[tag.text, { color: colors.red }]}>⛔ Expired</Text>
      </View>
    );
  if (days <= 3)
    return (
      <View style={[tag.wrap, tag.fire]}>
        <Text style={[tag.text, { color: '#EA580C' }]}>🔥 {days}d left</Text>
      </View>
    );
  if (days <= 7)
    return (
      <View style={[tag.wrap, tag.warn]}>
        <Text style={[tag.text, { color: colors.yellow }]}>
          ⚡ {days}d left
        </Text>
      </View>
    );
...
function ApplyModal({
  ...
  const applyMutation = useMutation({
    ...
    onError: (e: any) => {
      const msg =
        e.response?.data?.error || 'Failed to apply. Please try again.';
        ...
        <View style={modal.sheet}>
          ...
          <View style={modal.body}>
            ...
            <View style={modal.tipBox}>
              ...
              <Text style={modal.tipText}>
                • Reference a specific skill or trait
              </Text>
              <Text style={modal.tipText}>
            ...
            <View style={modal.actions}>
              ...
              <TouchableOpacity
                style={[
                  modal.submitBtn,
                  applyMutation.isPending && modal.submitDisabled,
                ]}
  ...
  return (
    <View style={[card.wrap, isUrgent && card.urgent]}>
      <View
        style={[
          card.stripe,
          { backgroundColor: isOpen ? colors.brand[600] : colors.gray[300] },
        ]}
      />
        ...
        <View style={card.topRow}>
          ...
          <View
            style={[
              card.statusBadge,
              {
                backgroundColor: isOpen ? colors.greenLight : colors.gray[100],
              },
            ]}
          >
            <Text
              style={[
                card.statusText,
                { color: isOpen ? colors.green : colors.gray[500] },
              ]}
            >
              {isOpen ? '● Open' : '○ Closed'}
            </Text>
...
export function CastingCallsScreen() {
  ...
  const openCount =
    data?.items?.filter((i: any) => i.status === 'open').length ?? 0;
  const urgentCount =
    ...
    <View style={s.container}>
      ...
      {isTalent && (
        <View style={s.roleBanner}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.roleBannerSub}>
              Tap "Apply Now" on any open role to submit your application
            </Text>
          </View>
        </View>
      )}
      {isCasting && (
        <View
          style={[
            s.roleBanner,
            { backgroundColor: colors.blueLight, borderColor: '#BFDBFE' },
          ]}
        >
          <Text style={s.roleBannerIcon}>🎥</Text>
          <View style={{ flex: 1 }}>
            <Text style={[s.roleBannerTitle, { color: colors.blue }]}>
              You're a Casting Director
            </Text>
            <Text style={s.roleBannerSub}>
              These are live casting calls. Post yours from your dashboard.
            </Text>
          </View>
        </View>
      )}
      ...
      {!isLoading && (data?.total ?? 0) > 0 && (
        <View style={s.statsRow}>
          ...
          <View style={s.statDivider} />
          <View style={s.statBox}>
            <Text style={[s.statNum, { color: colors.yellow }]}>
              {urgentCount}
            </Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <CastingCard
              ...
              canApply={isTalent}
              onApply={() =>
                setApplyTarget({ id: item._id, title: item.title })
              }
            />
          )}
