    ...
    >
      <View style={[featured.photo, { backgroundColor: bg }]}>
        {talent.profilePhoto?.url ? (
          <Image
            source={{ uri: talent.profilePhoto.url }}
            style={featured.img}
          />
        ) : (
        ...
        <View style={featured.overlay}>
          <View style={featured.starBadge}>
            <Text style={featured.starText}>⭐ Featured</Text>
          </View>
        </View>
        {talent.isVerified && (
      ...
      <Text style={featured.name} numberOfLines={1}>
        {talent.fullName}
      </Text>
    ...
    >
      ...
      <View style={[card.photo, { backgroundColor: bg }]}>
        ...
        <View
          style={[
            card.dot,
            { backgroundColor: isAvail ? '#10B981' : '#9CA3AF' },
          ]}
        />
      ...
      <View style={card.info}>
        ...
        {talent.categories?.[0] && (
          <View style={card.tag}>
            <Text style={card.tagText}>
              {talent.categories[0].replace('_', ' ')}
            </Text>
          </View>
        )}
      </View>
...
export function MarketplaceScreen({ navigation }: any) {
  ...
  const { data, isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: () =>
      api
        .get('/talents', {
          params: { search, page, limit: 20, category: cat.value },
        })
        .then(r => r.data.data),
    staleTime: 0,
  });
          ...
          >
            {featuredItems.map((t: any) => (
              <FeaturedCard
                ...
                talent={t}
                onPress={() =>
                  navigation.navigate('TalentProfile', { slug: t.slug })
                }
              />
            ))}
          </ScrollView>
