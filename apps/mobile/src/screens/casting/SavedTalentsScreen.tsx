...
export function SavedTalentsScreen({ navigation }: any) {
  ...
  const removeMutation = useMutation({
    mutationFn: (profileId: string) => api.delete(`/casting/me/saved/${profileId}`),
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyWrap}>
              ...
              <Text style={styles.emptyDesc}>Browse the marketplace and save talents you love.</Text>
            </View>
          }
          renderItem={({ item }) => (
            <TouchableOpacity
              style={styles.card}
              onPress={() => navigation.navigate('TalentProfile', { slug: item.slug })}
            >
              <View style={styles.photo}>
                {item.profilePhoto?.url ? (
                  <Image source={{ uri: item.profilePhoto.url }} style={styles.photoImg} />
                ) : (
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: colors.white, padding: spacing.lg, paddingTop: 56, borderBottomWidth: 1, borderBottomColor: colors.gray[100] },
  title: { ...typography.h2, color: colors.gray[900] },
  sub: { ...typography.caption, color: colors.gray[500], marginTop: 2 },
  list: { padding: spacing.md, gap: spacing.sm },
  card: { backgroundColor: colors.white, borderRadius: radius.lg, flexDirection: 'row', alignItems: 'center', padding: spacing.md, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 8, elevation: 2, gap: spacing.md },
  photo: {},
  photoImg: { width: 56, height: 56, borderRadius: radius.md },
  photoFallback: { backgroundColor: colors.brand[100], justifyContent: 'center', alignItems: 'center' },
  ...
  tag: { backgroundColor: colors.brand[50], borderRadius: radius.full, paddingHorizontal: 8, paddingVertical: 2 },
  tagText: { fontSize: 11, color: colors.brand[700], textTransform: 'capitalize' },
  emptyWrap: { alignItems: 'center', paddingTop: 60 },
  emptyTitle: { ...typography.h3, color: colors.gray[700], marginTop: spacing.md },
